...
void setupDirectory(std::filesystem::path current_path,
                    std::size_t directories_per_depth,
                    std::size_t files_per_directory,
    ...
    for (std::size_t dir = 0; dir < directories_per_depth; dir++) {
        ...
        if (current_recursion_depth < recursion_depth) {
            ...
            setupDirectory(current_path / dir_name, directories_per_depth, files_per_directory,
                           recursion_depth, current_recursion_depth + 1);
        }
    }
}

constexpr std::size_t calcualteTotalDirectoryCount(std::size_t recursion_depth,
                                                   std::size_t directories_per_depth) {
    // For each level of recursion, the number of directories created is equal to
    // directories_per_depth. Therefore, the total number of directories created at a given
    // recursion depth d is directories_per_depth^d. The total number of directories created for the
    // entire tree is the sum of the number of directories created at each level of recursion.
    // Therefore, the total number of directories created is:
