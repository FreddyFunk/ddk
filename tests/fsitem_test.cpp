...
class FSItemTest : public testing::Test {
  protected:
    FSItemTest() { std::filesystem::create_directory(base_path); }
...
TEST_F(FSItemTest, DuplicateTest) {
    ...
    const auto file_0_it = std::find_if(children.begin(), children.end(), [](const auto &child) {
        return child->getItemName() == "file_0.txt";
    });
    auto file_0 = children.at(std::distance(children.begin(), file_0_it));

    const auto file_0_copy_it =
        std::find_if(children.begin(), children.end(),
                     [](const auto &child) { return child->getItemName() == "file_0_copy.txt"; });
...
class FSItemSymlinkTest : public testing::Test {
  protected:
    FSItemSymlinkTest() {
        ...
        Data::setupDirectory(symlink_dir_path, directories_per_depth_symlink,
                             files_per_directory_symlink, recursion_depth_symlink);
        file_size_symlink =
            std::filesystem::file_size(symlink_dir_path / ("file_" + std::to_string(0) + ".txt"));
        ...
        try {
            ...
            std::filesystem::create_symlink(symlink2_dir_path / file_name_symlink2,
                                            base_path / file_name_symlink2);
        } catch (const std::system_error &e) {
            // Windows specific error: "create_symlink: A required privilege is not held by the
            // client."
            if (e.code().value() == 1314) {
                system_error = "\"create_symlink: A required privilege is not held by the "
                               "client.\" This is a common issue when testing symlinks on Windows. "
                               "Try executing this test as an administrator.";
    ...
    const std::filesystem::path symlink_dir_path =
        std::filesystem::current_path() / "" / symlink_dir;
    ...
    const std::size_t total_directory_count_symlink =
        Data::calcualteTotalDirectoryCount(recursion_depth_symlink, directories_per_depth_symlink);
    const std::size_t total_file_count_symlink =
        (total_directory_count_symlink * files_per_directory_symlink) + files_per_directory_symlink;
    std::size_t file_size_symlink = 0;

    // Setup file that will be linked
    const std::string symlink2_dir = "ddk_test_data_symlink2";
    const std::filesystem::path symlink2_dir_path =
        std::filesystem::current_path() / "" / symlink2_dir;
...
TEST_F(FSItemSymlinkTest, IgnoreSymlinks) {
    ...
    EXPECT_EQ(item->getItemType(), std::filesystem::file_type::directory)
        << "for path: " << base_path;
...
TEST_F(FSItemSymlinkTest, SymlinkFileAndSymlinkDirectory) {
    ...
    EXPECT_EQ(item->getItemType(), std::filesystem::file_type::directory)
        << "for path: " << base_path;
    EXPECT_EQ(item->getSizeInBytes(), file_size * total_file_count +
                                          file_size_symlink * total_file_count_symlink +
                                          file_size_symlink2);
    EXPECT_EQ(item->getItemName(), test_directory);
    EXPECT_EQ(item->getPath(), base_path);
    EXPECT_EQ(item->getPathAsString(), base_path.string());
    EXPECT_EQ(item->getChildFilesCount(), total_file_count + total_file_count_symlink + 1);
    EXPECT_EQ(item->getChildSubDirectoriesCount(),
              total_directory_count + total_directory_count_symlink + 1);
...
// The fixture for testing class FSItem.
class FSItemTestFileSystemStructures
  ...
  protected:
    ...
    ~FSItemTestFileSystemStructures() override { std::filesystem::remove_all(base_path); }

    void SetUp() override { item = new FileSystemItem(base_path, nullptr); }
...
TEST_P(FSItemTestFileSystemStructures, CorrectInitializedChildSubdirectory) {
    ...
    const auto it = std::find_if(children.begin(), children.end(),
                                 [](const auto &item) { return item->getItemName() == "dir_0"; });
    auto sub = children.at(std::distance(children.begin(), it));

    const std::size_t sub_directory_count =
        Data::calcualteTotalDirectoryCount(recursion_depth - 1, directories_per_depth);
    const std::size_t total_sub_directory_file_count =
        (sub_directory_count * files_per_directory) + files_per_directory;
    ...
    EXPECT_EQ(sub->getItemType(), std::filesystem::file_type::directory)
        << "for path: " << sub->getPath();
    ...
    EXPECT_EQ(sub->getChildren().size(),
              (recursion_depth == 1 ? 0 : directories_per_depth) + files_per_directory);
...
TEST_P(FSItemTestFileSystemStructures, CorrectInitializedChildFile) {
    ...
    const auto children = item->getChildren();
    const auto file = std::find_if(children.begin(), children.end(), [](const auto &item) {
        return item->getItemName() == "file_0.txt";
    });
    ...
    EXPECT_EQ(sub->getParent(), item);
    EXPECT_EQ(sub->getRelativeDirDepth(), 1);
    EXPECT_EQ(sub->getItemType(), std::filesystem::file_type::regular)
        << "for path: " << sub->getPath();
...
TEST_P(FSItemTestFileSystemStructures, CorrectInitializedChildFileInSubdirectory) {
    ...
    const auto children = item->getChildren();
    const auto dir = std::find_if(children.begin(), children.end(),
                                  [](const auto &item) { return item->getItemName() == "dir_0"; });
    auto sub_dir = children.at(std::distance(children.begin(), dir));

    const auto sub_dir_children = sub_dir->getChildren();
    const auto file =
        std::find_if(sub_dir_children.begin(), sub_dir_children.end(),
                     [](const auto &item) { return item->getItemName() == "file_0.txt"; });
    ...
    EXPECT_EQ(sub->getPathAsString(),
              (base_path / sub_dir->getItemName() / sub->getItemName()).string());
    EXPECT_EQ(sub->getChildFilesCount(), 0);
    EXPECT_EQ(sub->getChildSubDirectoriesCount(), 0);
    EXPECT_EQ(sub->getChildSymlinksCount(), 0);
    EXPECT_EQ(sub->getChildren().size(), 0);
